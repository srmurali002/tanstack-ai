...
async function testBedrockToolCalling() {
    console.log('Testing Bedrock tool calling (Claude 4.5 Haiku)\n')
    ...
    try {
        const result = await chat({
            adapter,
            messages: [{ role: 'user', content: 'What is the temperature in London in celsius?' }],
